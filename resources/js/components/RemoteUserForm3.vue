    ...
    <div class="h-100 w-100">
        ...
        <b-form @submit.prevent="sendData">
            ...
            <div v-else class="d-flex align-items-center justify-content-center flex-column">
                ...
                <span v-if="errors.recaptcha_token" class="text-danger">{{ errors.recaptcha_token }}</span>
            </div>
        </b-form>
    </div>
</template>
    ...
    methods: {
        ...
        async sendData() {
            ...
            return new Promise(async (resolve, reject) => {
                if(!window.grecaptcha || !window.RECAPTCHA_SITE_KEY) {
                    alert('reCAPTCHA nije dostupna, pokusajte ponovo!');
                    reject(error);
                }
                ...
                if(this.userId != null && this.userId != '') {
                    ...
                    // Deo koji se tice reCAPTCHA
                    const token = await window.grecaptcha.execute(window.RECAPTCHA_SITE_KEY, { action: 'contact_submit' });
                    formData.append('recaptcha_token', token);
                    // formData.append('recaptcha_action', contact_submit);

                    this.formAction = '/anonimous/create3';
                }
